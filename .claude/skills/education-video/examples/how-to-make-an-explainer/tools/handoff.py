...
def main() -> int:
    ...
    w("")
    w("| scene | title | in | out | duration | pause before first word |")
    w("|---|---|---|---|---|---|")
    for s in scenes:
        w(f"| {s['scene']} | {s['title']} | {clock(s['in_s'])} | {clock(s['out_s'])} | "
          f"{s['duration_s']:.1f} s | {s['pause_s']:.2f} s |")
    w("")
    w("**Scene boundaries came from the audio and are contiguous** — each scene ends exactly where the")
    w(f"next begins, and the table sums to the file duration. The pause at the top of every scene is")
    ...
    w("- **No tolerances, stage names, checklists internals, loudness figures or engineering plumbing.**")
