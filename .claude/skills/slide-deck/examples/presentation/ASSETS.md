# Asset plan (rev 6, 18 slides — the problem stated first at STAGE 4, user's call: real community evidence)
...
| 4 | the shared stage shape | universal pipeline: interview → posted checklist → stages, a you-approve loop on every stage; the source audit highlighted as the first stage everywhere | drawn |
| 5 | technical-report | its pipeline as a stage diagram: evidence map → skeleton → sections → cold read; cold reader shown testing the judgement playback | drawn |
...
| 8 | EXAMPLE: this deck, at the start | **the entire editor window**, uncropped: this repository in the tree, the run in the chat panel, one sentence typed and six stages back with nothing ticked | **user** (screenshot, whole) |
| 9 | EXAMPLE: the same run, mid-flight | the same window later: STAGE 0 and 1 ticked and naming their files, STAGE 2 marked *here*, 3–5 open | **user** (screenshot, whole) |
...
- Source-first: the highlighted first stage in every pipeline diagram (slides 4–5, 7, 12, 15).
...
- Act IV (no-overclaiming, human-judgement) removed whole at the user's direction during
  STAGE 3; both themes already ride in the notes of slides 5, 12 and 16 and in the pipeline
...
- ~~The two YouTube URLs~~ — **landed during STAGE 4**: education
...
## The screenshots (landed at STAGE 4, 2026-08-19)
...
| 9 | `deck-run-stage2-original.png` (whole) | 1306px, 0.34× | ~13px |
| 10 | `deck-run-stage5-original.png` (whole) | 1306px, 0.34× | ~13px |

**These four slides deliberately break the STAGE 0 28px floor**, at the user's explicit
...
The legible crops from rev 4 are kept — `slide7-checklist-stage0.png` (1240px, 28.2px text)
and `slide8-checklist-stage2.png` (1700px, 39px text) — so the call can be reversed in one
