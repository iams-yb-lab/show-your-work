...
## Pass 1 — 2026-08-19, fresh subagent, slides only
...
**Argument playback:** faithful, in order, act by act. Viewer's own remark: "I could
reconstruct the whole pitch from headlines alone."

**Claims taken as measured:** the two films' specs (4:30/8 scenes/55 lines; 84 s/2520
frames/2560×1440), draft-3-first-viewing, 421 voided frames, 10.5 h guess vs 16.1 h
measured, and slide 7's own checklist/storyline artifacts. All of these ARE measured or
...
(12), CAD-checklist chips incl. DNP (11), the 421-frames caption arriving before its
story (11).
...
accused the viewer of shifted numbering and an invented quote. Wrong on both counts: a
parallel session of this run had reordered Act III (proofs first) and reworded the
explainer-proof headline — "came out of the workflow on the next two slides" — and
rebuilt the PNGs before the viewer read them. The viewer's numbering and quote were
faithful to the deck it actually saw (the 15-slide rev with Act III reordered). The
accusation is retracted; the viewer was right.

**Speaker-notes coverage check (for the user's call):** of the questions above, these are
already answered aloud in the existing notes: natural-voice's function (slide 2 + 8
...
the speaker's notes answer aloud (talk deck; the room has a speaker, the cold viewer did
...
The rehearsal above was it: end to end, aloud, as the room will get it. Nothing surfaced.
**Run closed by the user 2026-08-19.**
...
- **The cold pass no longer covers the deck.** Pass 1 viewed the 15-slide rev. Slides 2, 8,
...
- **The timing pass is stale.** It was rehearsed at 15 slides; there are now two more, both
  carrying speaker notes of ordinary length.

**Not re-run here.** A fresh cold viewer and a fresh timing pass are what the skill asks for
after a change of this size. Recorded, not done — the user's call whether to spend them.

---

## Delivered — 2026-08-19

Presented to the room at 18 slides, the rev the two stale passes above never
covered. **The speaker's report: it went well.**

That is the whole of it, and it is recorded as what it is. Nothing was measured
in the room: no wall-clock against the ~15-minute budget, no note of whether the
four sub-floor screenshot slides read from the back, no list of what was asked
afterwards. So the room confirms the deck worked; it does not retire the
specific doubts the reopening raised, and it cannot be cited as if it had.

**What this does to the open item above.** The cold pass and timing pass stay
un-re-run, and now they will not be — a live delivery has overtaken them, and
re-rehearsing a deck that has already been given to its audience buys nothing.
The honest state is: 15-slide cold pass on record, 18-slide delivery on record,
nothing formal in between. Both are above, and neither is dressed up as the
other.

**If this deck is given again**, the cheap thing that would close the gap is to
time it and write down the questions — one pass, in the room, no rehearsal
needed. The leftover-questions list from pass 1 is the natural checklist to hold
it against.
