# Source audit — STAGE 1

Audited 2026-08-18 against the STAGE 0 source list. Every deck claim must trace to a line in
...
| `.claude/skills/technical-report/SKILL.md` | **specified** (method distilled from a produced report that lives with its project, not here) | order: evidence → skeleton → sections → cold read; 6 stages; delivers evidence map + report + cold-read record |
| `.claude/skills/slide-deck/SKILL.md` | **specified — no produced deck behind it**; states this itself | order: source → storyline → slide scripts → build → cold pass; 6 stages; delivers storyline + scripts + master + cold-pass record. The deck must carry this caveat where it leans on the skill — and this deck is itself the skill's first evidence |
| `.claude/skills/education-video/SKILL.md` | **specified + produced evidence** (its reference film: 9 scenes, 49 lines — numbers live with that film, elsewhere) | order: document → script → audio → picture; 6 stages; delivers source doc + script + audio master + image manifest + one Claude Design prompt; Claude Design draws the picture, rendering is ours; natural-voice mandatory at audio |
...
| Three screenshots of this deck's own run, supplied by the user 2026-08-19 (`presentation/assets/deck-run-original.png`, `deck-run-stage2-original.png`, `deck-run-stage5-original.png`) | **produced evidence of the run itself** — added at STAGE 4, after this audit; unretouched captures, crops only, nothing composited or retyped | the same run at three moments: posted with all stages open, two stages in, and closed with six ticks and every artifact named. Shown whole and uncropped at the user's direction, so what they may **not** be taken for is *current*: the "26 slides / agreed 25–28" on the second and the "16 slides" on the third were true when captured and are stale now. Corrections are spoken in the notes; the pixels stay untouched |

| Two public r/ClaudeCode posts, screenshotted by the user 2026-08-19 (`presentation/assets/reddit-unreadable-jargon.png`, `reddit-word-salad.png`) | **published third-party criticism**, unretouched, attributed on the slide; added at STAGE 4 after this audit | that people find raw model output unreadable — "unreadable jargon", "word salad", "impenetrable" — at 140 and 316 upvotes. **The hard limit:** neither post is about a generated *report* or *deck*. They may not be used to claim that AI-written documents are slop; slide 2's headline is written to claim only legibility, and must not be upgraded in the room |
...
- **Namesakes — two hazards found, naming rule set:**
  1. *Two explainer films.* education-video's reference film (9 scenes, 49 lines, lives
     ...
     their numbers.
  2. *"STAGE n" means a different thing in every skill.* The deck never says a bare "STAGE 3";
     stages are always qualified by skill, or discussed as the shared pattern.
...
- **Scope limits explicit** — yes, and the deck inherits them: skills are read-only; films
  ...
  human judgement outranks measurement; `~/.claude/skills/` install refused.
- **Judgement** — the deck ends on the STAGE 0 takeaway, not a comparison judgement; the source
  needs no losers' reasons.

## Ruling

The source is ready. No document fix required before STAGE 2.

## Amendment — STAGE 4, 2026-08-19
...
## Second amendment — STAGE 4, 2026-08-19
