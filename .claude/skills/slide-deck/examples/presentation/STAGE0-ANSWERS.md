# STAGE 0 answers — how-to-use-the-skills deck

Interviewed 2026-08-18. These answers are the deck's contract; STAGES 1–5 check against them.
...
## Speaker
The repo owner (the user). Notes are written in their voice. STAGE 5's timing pass is
...
**Must be taught:** that the four output skills exist and what each one produces; the stage
...
where the files go); that the skills are read-only; that human ears and eyes
outranks every measurement.

## The one takeaway
> "Name the output, answer the interview, and the stages carry you to a delivered report,
> deck, or video."

Lands as the final headline. STAGE 5's cold viewer is judged against it.

## Length and slide budget
~15+ minutes spoken. **Agreed budget: revised at STAGE 2 to 16–18 slides** (user's call:
...
**Agreed font-size floor: 28px on the 1080p canvas** (≈21pt). STAGE 4 checks nothing
...
Committed at STAGE 4; no dark-mode variant.

**Style reference (added during STAGE 3, user's direction):** the intro film's picture —
...
**Diagram-heavy (added during STAGE 2):** the deck is mainly about convenient proven
workflows, so most slides carry an embedded diagram — pipelines, stage maps, timelines —
drawn as inline SVG in the master. Asset plan lives in `ASSETS.md`; user-gathered images
land in `presentation/assets/`.

## STAGE 5 measured pace
