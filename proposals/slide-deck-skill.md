# Proposal: a fifth skill — `slide-deck`

**Status: APPLIED 2026-08-18.** The user authorized with the exact phrase. The verified text
below is now live at `.claude/skills/slide-deck/SKILL.md`, extracted from this proposal's fenced
block programmatically rather than retyped, so it is byte-identical by construction — hash
`1ecd48bc86bef336` blessed into `tools/skill-hashes.txt`, the addition recorded in
`EXPORT-MANIFEST.md`, the row and the five-skill counts updated in `README.md` and `CLAUDE.md`.
This document remains as the rationale, the analysis, and the verification record.

**Instated, mechanically** (the `technical-report` route, verified 2026-08-18):

1. Create `.claude/skills/slide-deck/SKILL.md` with the fenced text below, byte-for-byte.
2. `python tools/check_links.py --bless` — a new skill file not in `tools/skill-hashes.txt`
   otherwise fails the `skills` check as "present but not recorded".
3. Record the addition in `EXPORT-MANIFEST.md` under "Added after export" — like
   `technical-report`, this skill originates here and has no source-repository counterpart.
4. Add the row to `README.md`'s skill table and update the "four skills" counts in `README.md`
   and `CLAUDE.md` to five; note this is the second skill with no `video/` dependency.
5. `python tools/check_links.py` — all five checks must pass.

`install_skills.py` copies `.claude/skills` whole, so it needs no change.

## Why

`education-video` makes films, `technical-report` makes standing documents, `showoff-render`
makes hero footage. The remaining way a project is routinely presented — a slide deck, spoken
over in a room or read alone afterward — has no skill, so every deck is improvised and the
family's protections (audited source, guarded structure, cold test, user-owned gates) do not
apply to it. The same failure the siblings guard against exists here in deck form: slides drawn
before the story is right get redrawn after every change of story, and a polished deck that
overclaims cannot be recalled from the room that saw it.

## Provenance — stated honestly

**This skill has no produced reference deck.** The user confirmed this on 2026-08-18. What it
inherits, and from where:

- **The run protocol** — interview, posted checklist, named gates, user-owned approvals,
  stop-the-run, tick-and-repost, close-with-one-question — carried faithfully from the three
  gate-run siblings (`natural-voice` carries no gate protocol), which do have evidence behind
  it.
- **The source-document standard** (GATE 1) from `education-video` GATE 1 and `technical-report`:
  numbers with homes, statuses labelled, mechanism as causal chain, namesakes disambiguated.
- **The honesty rules** (traces, rounding-only, claims wearing status inline) from
  `education-video` GATE 2 and `technical-report` GATE 3.
- **The slide craft itself** — assertion headlines, one claim per slide, the flip test — is
  established practice (assertion-evidence design), not a measurement made in this repository.
  The skill says so about itself, and says the first deck it produces becomes its first
  evidence, logged with that deck, never here.

## Decisions made by the user, 2026-08-18

1. **Both modes, chosen at GATE 0.** A deck is spoken over (talk deck) or read alone (reading
   deck); the interview asks, and GATES 3 and 5 branch on the answer.
2. **HTML is always the build medium.** The master is one self-contained HTML file; the user
   picks the final export at GATE 0. The honest conversion facts are encoded in the skill:
   HTML → PDF is faithful (browser print); HTML → PowerPoint is *not* lossless — either
   pixel-faithful image slides with real notes text, or a hand-editable rebuild that is a second
   implementation and is priced as one. The skill never promises an editable pptx that is "the
   same file as" the HTML.
3. **No reference deck** — provenance as above.

## The pipeline, and where each gate comes from

| gate | what it guards | sibling precedent |
|---|---|---|
| GATE 0 — the interview | mode, audience, takeaway, budget, export, home | all four; the two-list audience contract from `technical-report` |
| GATE 1 — the source | a deck is only as accurate as the document under it | `education-video` GATE 1, which also names help-them-write-it-first as the work; `technical-report` is the skill that writes it |
| GATE 2 — the storyline | the argument, before any slide exists | the guarded gate, like the script / the skeleton / the camera freeze |
| GATE 3 — the slide scripts | what each slide shows and says, traced, verified before shown | `technical-report` GATE 3's draft-verify-show-stop, act-by-act instead of section-by-section |
| GATE 4 — the build | the HTML master, checked mechanically, then the export | `education-video`'s cross-check-it-mechanically; showoff's point-each-check-at-one-artifact |
| GATE 5 — the cold pass, then the user's pass | the audience claim tested on someone who has none; the bench | `technical-report` GATES 4–5 merged, plus a talk-mode timing pass |

**Why the storyline is the guarded gate.** The expensive half of a deck is everything built on
the order of the argument: the acts, the traces, the slides themselves, the build. A storyline
change after GATE 3 opens re-cuts the acts, re-traces claims, and strands slides the user
already approved — the same economics that make `education-video` guard the script and
`technical-report` guard the skeleton.

**Why GATE 5 merges the cold read and the user's pass** where `technical-report` splits them:
a deck is one sitting's artifact — minutes, not a document read over days — so the cold pass,
its fixes, and the user's own end-to-end pass fit one gate without the run losing its place.
The stop-the-run and reopen rules still apply within it.

**Why the timing pass uses the speaker's measured pace, not a book value.** This repository has
no measurement of any speaker, and the bench outranks every number in here. So the skill orders
a measurement — the speaker reads one act aloud and is timed — rather than shipping a wpm
constant it has no evidence for.

## What this does not change

`education-video`'s "you deliver five things" section still forbids making a film's picture
"as slides" — a film is not a deck and this skill does not touch that rule. `natural-voice` is not implicated: a talk deck's
speaker is a human, and nothing here generates speech.

---

## Proposed text for `.claude/skills/slide-deck/SKILL.md`
...
1. **The slide budget rule.** GATE 0 says "proposed from the answer and agreed here, never
   defaulted" but deliberately prescribes no slides-per-minute constant — this repository has no
   evidence for one. If runs drift, the first produced deck's log is where the number comes
   from.
2. **The act size.** Unprescribed, like the evidence map's format in `technical-report` —
   grouping is required, the granularity is not.
3. **The cross-check tool's shape.** The contract is fixed (headlines + slide count, storyline
   vs master, non-zero on difference); the implementation is per-deck, like `education-video`'s.
4. **Reading-deck timing.** No timing pass in reading mode — reading minutes were asked at
   GATE 0 but no mechanical check is proposed. Add one only if a produced deck shows the need.

## Verification — 2026-08-18

Two independent reviewers examined the skill text before instating; their findings were applied
to the fenced text above.

**Consistency reviewer** (against the four siblings, `CLAUDE.md`, `README.md`, and the
`technical-report` precedent): the "Never edit a skill" section verified byte-identical to the
canonical form by hash; frontmatter, gate numbering, wrapper protocol elements, standalone rule
(no absolute path, drive letter, or named external project), where-do-files-go rule, and family
register all passed. Findings fixed: the GATE 5 cold-pass loop had no end condition (the merge
of `technical-report`'s GATES 4–5 dropped its closure sentence — restored, adapted); the
stop-the-run reservation ("not a stop-the-run event; stop-the-run stays for new direction") was
missing from GATE 5 — restored; the mechanical cross-check appeared in the four-things list but
not in GATE 5's delivery — delivery list aligned; GATE 1's close broke close-with-one-question
without being a named exception — added to the exceptions sentence. In this document: the
provenance bullet overstated "word-for-word from the four siblings" (now: faithfully, from the
three gate-run siblings); a rule misattributed to `education-video` GATE 5 (now: its "you
deliver five things" section); the gate table's GATE 0 row drifted from the fenced checklist
(now carries "export").

**Executability reviewer** (simulating a fresh session obeying the text literally). Blockers
found and fixed:

- *Speaker notes had no specified home in the master*, so GATE 5's notes-free cold-pass
  artifact could not be produced, and the cold viewer reading HTML source would have seen
  hidden text the room never sees. Fixed: the notes live in the master, marked as notes; the
  master renders without them; that notes-free rendering is what the room sees, what exports
  derive from, and what the cold viewer gets.
- *GATE 5 could not close*: every claim-changing fix demanded a fresh cold viewer, who always
  returns questions. Fixed: the cold passes end when a fresh viewer's playback changes nothing;
  leftover questions are explicitly the user's call; only then does the user's pass open.
- *A GATE 5 fix that changed a headline wedged the run*: the cross-check would fail against the
  approved storyline and no rule authorized editing it. Fixed: a headline fix is a storyline
  change even at GATE 5 — named, user-approved, storyline updated, flip test re-run, then the
  cross-check.

Should-fixes applied: the font-size floor now gets proposed as a number and agreed at GATE 0
(GATE 4's check was otherwise unrunnable); the timing pass names who times (the user or the
speaker), writes the measured pace with the GATE 0 answers, and states the unreachable-speaker
path — never a book value silently; the headline byte-identity vs status-inline collision
resolved by putting the status word into the storyline headline at GATE 2; the slide script
gained an explicit trace slot (verification demanded traces the three-part structure gave no
home); the GATE 4 close now aims a human at the export too, not only the master (a print can
split a slide the master shows whole); a talk deck that leaves the room gets its notes' vehicle
agreed at GATE 0 as part of the export choice.

Nits applied: the intro's "log" named as the GATE 0 answers plus the cold-pass record; the
storyline file's format constrained for the parser (one headline per line, marked act lines,
ticks never in the file); GATE 3's intra-gate discovery reworded from "is a stop-the-run event"
to "treat that like a stop-the-run" (the wrapper defines stop-the-run for approved gates); the
pptx image-export's "real notes text" scoped to "where notes exist" (reading decks have none)
and the font/overflow checks given the same build-the-tool duty as the cross-check; the slide
budget's "derived" (an uncomputable promise) replaced with "proposed … and agreed".

**Accepted as-is, deliberately** (inherited from the siblings verbatim, so changing them here
would be protocol drift): the "GATE n of 5" arithmetic that counts gates 1–5 with GATE 0
outside the count, and the plan-posting message existing before any gate has opened.
