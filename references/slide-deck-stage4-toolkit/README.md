# Reference implementation — the four STAGE 4 mechanical checks

Supporting code for the STAGE 4 proposal, which was **applied on 2026-08-23 and then deleted**, as an
...
| file | what it is |
|---|---|
| `mechcheck.js` | the four checks, run inside a built HTML master, in the browser, where real geometry can be measured. Writes its judgement to `<div id="mechcheck">` and reddens `#mechbanner` on failure |
| `run_mechcheck.py` | loads the master in headless Chrome, reads that judgement, exits non-zero on any finding |
