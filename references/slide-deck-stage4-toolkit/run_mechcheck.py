...
Loads the master in headless Chrome, lets mechcheck.js run, reads the judgement
...
def main():
    ...
    if not m:
        print("could not find #mechcheck in the rendered DOM — is mechcheck.js inlined?",
              file=sys.stderr)
        return 2
    judgement = htmlmod.unescape(m.group(1)).strip()
    print(judgement)
    return 1 if judgement.startswith("MECHCHECK FAIL") else 0
